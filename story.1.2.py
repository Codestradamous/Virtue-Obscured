...
#A class to hold the actions so that they may repeatedly be called when attempting an action at a user choice point.
#here is a commented out sample of how I might have to change the code to include the randomizer.
##good_answer = 'A) attack'
##p = raw_input("raw test")
##letters = 'asied'
##
##
##def filtering():    
##    x = p
##    for var in letters:
##        if var == good_answer[3] and x == 'a' or x == 'A':
##            print "weeee"
##        else:
##            print "not so fast"
##
...
## temporarily(maybe permanently) on hold as I come up with the next class
##import abc
##
...
# This class will allow instances to be created that will be able to change the life meter in different amounts
#depending on the encounter scenario as I do plan to have scenarios where specific actions will incur greater
#or lesser penalties/rewards.
...
class life_modifier(action):
    ...
    def outcome(self):       
        ...
        elif x == 'neutral success':
            stats['life'] += self.neutral_success
        elif x == 'neutral failure':
...
Do you...''' 
good_choice = "A) Turn around and swing your sword!"
evil_choice = "B) Get out of the way! No reason not to duck, your brother got himself into this mess.
#Here is organization of the most common letters to start a sentence in English as I plan to use these letters as a reference for the program to identify
#which reader choice corresponds to what action and so that I can randomize the choices so as to avoid becoming predictable. This becomes a problem
#since it won't take a smart reader very long to figure out that answer D will always get you the charm. (Mental note, program charms.)

#Here are the letters from highest frequency to lowest. T  I A H S W B M O F N P C D E Y L R G J U V K Q Z X

#Since the code will reference the first letter in the text variable the lists are as follows
#These will take the place of "I think I will go with... A) B) C) D)"
#good_action_attempt tmocluz
#neutral_action_attempt ibfdrvx
#evil_action_attempt awnegk
#charm_answer_attempt hspyjq
